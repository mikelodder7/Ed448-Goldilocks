...
mod tests {
    ...
    fn test_is_on_curve() {
        ...
        let point = AffinePoint { x, y }.to_edwards();
        assert_eq!(point.is_on_curve().unwrap_u8(), 1u8);
    }
    #[test]
    fn test_compress_decompress() {
        ...
        let decompressed_point = point.compress().decompress();
        assert!(<Choice as Into<bool>>::into(decompressed_point.is_some()));

        assert!(point == decompressed_point.expect("decompress generator"));
    }
    #[test]
